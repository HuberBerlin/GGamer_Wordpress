...
class METAPRESS_PAYMENTS_MANAGER {
    ...
    public function find_pending_payment_for_product($product_id, $spender_address) {
        ...
        $db_query = "SELECT * FROM $get_table_name WHERE product_id = '$product_id' AND payment_owner = '$spender_address' AND transaction_status != 'paid'";
    ...
    public function find_payment_with_hash($product_id, $spender_address, $hash) {
        ...
        $payment = null;
        $get_table_name = $wpdb->prefix . $this->table_name;
        $db_query = "SELECT * FROM $get_table_name WHERE product_id = '$product_id' AND payment_owner = '$spender_address' AND transaction_hash = '$hash'";
        $payment = $wpdb->get_results($db_query);
        if( ! empty($payment) && isset($payment[0]) && ! empty($payment[0]) ) {
            $payment = $payment[0];
        }
    ...
    public function get_all_payments($offset, $filters) {
        ...
        if( ! empty($filters) ) {
            ...
            if( isset($filters->address) ) {
              if( $filters_added ) {
                  $db_query .= " AND payment_owner = '$filters->address'";
              } else {
                  $db_query .= " WHERE payment_owner = '$filters->address'";
              }
              $filters_added = true;
            }
    ...
    public function get_wallet_address_payments($wallet_address, $offset, $filters) {
        ...
        $db_query = "SELECT * FROM $get_table_name WHERE payment_owner = '$wallet_address'";
    ...
    function find_network_explorer_url($network_slug) {
        ...
        $metapress_live_mode = get_option('metapress_live_mode', 0);
        if( empty($metapress_live_mode) ) {
          $metapress_supported_networks = get_option('metapress_supported_test_networks');
        } else {
