...
class MetaPress_Plugin_Content_Filter_Shortcodes {
    ...
    public function metapress_restrict_post_type_content_with_products( $content ) {
        ...
        $is_admin_viewing_page = false;
        $user_has_access = true;
        $access_token = null;
        if( is_user_logged_in() && current_user_can('manage_options') ) {
            $is_admin_viewing_page = true;
        }
        ...
        if ( ! empty($metapress_required_products) ) {
            ...
            if( ! $user_has_access ) {
                ...
                $metapress_restricted_content .= '<div class="metapress-login-notice"><p><small>*'.__('Accessing this content requires a', $wp_metapress_textdomain).' <a href="https://metamask.io/" target="_blank">MetaMask</a> '.__('account', $wp_metapress_textdomain).', or a browser wallet</small>.</p><button class="metamask-connect-wallet">'.__('Connect Wallet', $wp_metapress_textdomain).'</button></div>';
	...
	public function metapress_product_checkout_shortcode( $atts ) {
        ...
        public function metapress_restrict_entire_site_access() {
            ...
            if( ! empty($post) ) {
                ...
                // SETTING A PAGE TO REQUIRE SPECIFIC METAPRODUCTS OVERRIDES GLOBAL REQUIRED PRODUCTS
