...
class MetaPress_Custom_Post_Meta_Box_Manager {
    ...
    public function generate_product_meta_boxes( $post ) {
        ...
        <div class="rogue-meta-box">
            <label><?php _e('Is Membership', 'rogue-themes'); ?>
            <input type="checkbox" name="metapress_product_is_subscription" value="1" <?php checked(1, $product_is_subscription); ?> readonly disabled />
            <span class="description">This feature is coming in a future version of Web3 Access.</span>
            </label>
        </div>
    <?php }
    ...
    public function save_product_meta_data( $post_id ) {
        ...
        if(metapress_save_custom_metabox_data( $post_id, 'metapress_restrictions_meta_data_save_nonce', 'metapress_restrictions_meta_data_save' )) {
            if( isset($_POST['metapress_required_products']) ) {
                ...
                update_post_meta( $post_id, 'metapress_required_products', null);
            }
    ...
    public function generate_post_type_metapress_product_settings( $post ) {
        ...
        <div class="metapress-admin-section metapress-border-box">
            ...
            <?php if($metapress_restrict_site_access) { ?>
                <p class="metapress-admin-notice"><?php echo sprintf(__('<strong>Restrict Website Access is enabled</strong>. Selecting any MetaProducts below will override your global <a href="%s">Access Settings</a> required Products.', $wp_metapress_textdomain), admin_url('admin.php?page=metapress-access-settings')); ?></p>
            <?php } else { ?>
                <p class="metapress-admin-notice"><?php echo sprintf(__('<strong><a href="%s">Restrict Website Access</a> is disabled</strong>. Restrict this entire page by selecting MetaProducts below.', $wp_metapress_textdomain), admin_url('admin.php?page=metapress-access-settings')); ?></p>
            <?php } ?>
          <div class="metapress-admin-settings metapress-border-box metapress-width-600">
            <div class="metapress-grid metapress-setting">
              <div class="metapress-setting-title">
                <?php _e('Required MetaProducts', $wp_metapress_textdomain); ?>
              </div>
              <div class="metapress-setting-content metapress-align-right">
                  <?php if( ! empty($mp_products) ) {
                      ...
                      <p><?php _e('You have not created any MetaProducts.', $wp_metapress_textdomain); ?></p>
                  <?php } ?>
              </div>
            </div>
          </div>
      ...
      protected function sanitize_nft_array($array_data) {
          if( ! empty($array_data) && is_array($array_data) ) {
              foreach($array_data as $key => &$nft_data) {
                  ...
                  $nft_data['contract_address'] = sanitize_key($nft_data['contract_address']);
